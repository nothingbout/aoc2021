...
def add_memory(memories, memory, input_num):
    existing = memories.get(memory, 0)
    if existing == 0 or input_num < existing:
        memories[memory] = input_num

memories = {}
memories[(0, 0, 0, 0)] = 0

# 15989929946196 is too high
...
for instr_index in range(len(instructions)):
    ...
    for (memory, input_num) in memories.items():
        if instr[0] == 'inp':
            if memory[3] > 1000000000: continue 
            for i in range(1, 10) if input_num > 0 else range(1, 2):
