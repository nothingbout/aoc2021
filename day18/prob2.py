class Node:
    ...
    def copy(self):
        if self.is_num(): return Node(num = self.num)
        return Node(left_node = self.left_node.copy(), right_node = self.right_node.copy())
...
def add_pairs_and_reduce(aa, bb):
    sum = add_pairs(aa, bb)
    while explode_leftmost(sum) or split_leftmost(sum):
        pass
    return sum

max_magnitude = 0
for a in input_pairs:
    for b in input_pairs:
        if a == b: continue
        # print(a)
        # print(b)
        # print()

        sum = add_pairs_and_reduce(a.copy(), b.copy())
        magnitude = sum.magnitude()
        max_magnitude = max(max_magnitude, magnitude)

        print(f'{magnitude} {sum}')
