...
class Dungeon:
    def __init__(self, rooms, room_size = None, hallways = None):
        self.rooms = rooms
        ...
        self.room_size = room_size if room_size is not None else len(self.rooms[0])

    def copy(self):
        return Dungeon(self.rooms.copy(), self.room_size, self.hallways.copy())
    ...
    def is_room_full(self, i):
        return len(self.rooms[i]) >= self.room_size
...
def get_move_cost(dungeon, move):
    ...
    if is_from_room:
        ...
        steps = room_hallway - to_index if to_index <= room_hallway else to_index - room_hallway
        steps += dungeon.room_size - len(dungeon.rooms[from_index]) + 1
    else:
        room_hallway = dungeon.first_hallway_from_room(to_index)
        mover_type = dungeon.hallways[from_index][-1]
        steps = room_hallway - from_index if from_index <= room_hallway else from_index - room_hallway
        steps += dungeon.room_size - len(dungeon.rooms[to_index])
...
def is_solved(dungeon):
    for i in range(len(dungeon.rooms)):
        ...
        if len(room) != dungeon.room_size or room[0] != i: return False
    return True
